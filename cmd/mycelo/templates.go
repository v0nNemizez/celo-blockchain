...
func (e loadtestEnv) createEnv(workdir string, applyConfigOverrides func(*config.Config)) (*config.Environment, error) {
	cfg := &config.Config{
		ChainID:          big.NewInt(1000 * (1 + rand.Int63n(9999))),
		GenesisTimestamp: uint64(time.Now().Unix()),
		Mnemonic:         "miss fire behind decide egg buyer honey seven advance uniform profit renew", // config.MustNewMnemonic(),
	...
	goldBalances := make([]config.Balance, len(env.DeveloperAccounts()))
	for i, acc := range env.DeveloperAccounts() {
		cusdBalances[i] = config.Balance{acc.Address, config.MustBigInt("1000000000000000000000000")}
		goldBalances[i] = config.Balance{acc.Address, config.MustBigInt("1000000000000000000000000")}
	}
